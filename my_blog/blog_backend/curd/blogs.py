...
async def get_blog_list(db:AsyncSession,tag_id: Optional[int] = None,skip:int=0,limit:int=10):
    if tag_id is None:
        query = select(Blog,tag.name).outerjoin(Blog_tags,Blog.id==Blog_tags.blog_id).outerjoin(tag,tag.id==Blog_tags.tag_id).where(Blog.is_delete==False).offset(skip).limit(limit).order_by(Blog.create_time.desc())
    else:
...
async def get_blog_detail(db:AsyncSession,blog_id: int):
    ...
    result = BlogResponse(id=blog.id,title=blog.title,content=blog.content,user_id=blog.user_id,tags_name=tag_names)
    return result
