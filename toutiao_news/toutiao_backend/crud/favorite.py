...
async def get_favorite_list(
    ...
    count_result = await db.exexute(count_query)
    total = count_result.scalar_one()

    #获取收藏列表 - 连表查询 join() + 收藏时间排序 + 分页
    #select(查询主题模型类,字段别名).join(联合查询的模型类,联合查询的条件).where().order_by().offset().limit()
    # 别名 Favorite.created_at.label("favorite_time")
    offset = (page-1) * page_size
    #[
    #  (新闻对象,收藏时间,收藏id)
    # ]
    query = select(News,Favorite.created_at.label("favoite_time")
