...
from cache.news_cache import cache_related_news,cache_news_detail,get_cache_categories,set_cache_categories,get_cache_news_list,get_cached_news_detail,get_cached_related_news,set_cache_news_list
...
async def get_news_list(db:AsyncSession,category_id: int, skip: int = 0, limit: int = 10):
 ...
 if news_list:
  ...
  # ORM 转成Pydantic, 再转为 字典
  # by_alias=False 不适用别名, 保存 Python 风格,因为Redis数据是给后端用的
...
async def get_news_detail(db: AsyncSession, news_id: int):
 ...
 # 如果 查询到数据,存入缓存(不适用别名,保持数据库字段名)
 if news:
  # 构造新闻详情数据用于缓存(包含content字段)
